...
export default async function CategoryDetailPage(props: any) {
  ...
  const category = typedCategories.find((cat) => (typeof cat.slug === 'object' ? cat.slug?.current : cat.slug) === slug);
  ...
  const validBlogs = typedBlogs.filter(blog =>
    Array.isArray(blog.authors) && (blog.authors as any[]).every(author =>
      author && typeof author === 'object' && typeof (author as any)._id === 'string' && typeof (author as any).name === 'string'
    )
  );
