...
export function BlogCategoryList({ blogs, title, description }: BlogCategoryListProps) {
  ...
  const categoryGroups: CategoryGroup[] = blogs.reduce((groups, blog) => {
    if (!blog.categories || blog.categories.length === 0) {
      ...
      if (uncategorizedGroup) {
        ...
        groups.push({
          category: { _id: "uncategorized", name: "Uncategorized", slug: "uncategorized" },
          blogs: [blog]
        });
      }
    } else {
      ...
      if (category) {
        const existingGroup = groups.find(g => g.category._id === category._id);
        if (existingGroup) {
